...
public class ZipUtils {
     ...
     */
    public void zip(File[] files, File destZipFile, boolean deleteZipped) throws IOException {
        ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(destZipFile));
        for (File file : files) {
            if (file.isDirectory()) {
                zipDirectory(file, file.getName(), zos);
                if (deleteZipped) if (!FileUtils.delete(file))
                    Log.w(TAG, "Added dir could not be deleted, dir=" + file);
            } else {
                zipFile(file, zos);
                if (deleteZipped)
                    if (!file.delete()) Log.w(TAG, "Added file could not be deleted, file=" + file);
            }
        }
        zos.flush();
        zos.close();
